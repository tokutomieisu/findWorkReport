...
try {
    ...
    $query = "SELECT * FROM mt_student";
    $stmt = $dbh->query($query);
    // echo "検索指定なしor検索結果なしの場合に利用するsql文：", $query, "　";
    // クエリの実行(受け取った検索指定と一致するデータを抽出するためのもの)
    if (!empty($_POST['student'])) {
        $studentId = $_POST['student'];
        $query1 = "SELECT * FROM mt_student WHERE student_id LIKE '%$studentId%'";
...
<body>
    ...
    <section class="main">
        <div class="text">
            ...
            <div class="tb_flex">
                <a class="btn btn_add_width" href="#" role="button">学生を追加する</a>
                <div class="input-group mb-3">
                    <form action="student.php" method="post">
                        <?php
                        if (0 <= $count) {
                            echo "<input type=\"text\" placeholder=\"学生IDで部分一致検索　", $studentId, "\" name=\"student\" value=\"\">";
                        } else {
                            echo "<input type=\"text\" placeholder=\"学生IDで部分一致検索\" name=\"student\" value=\"\">";
                        }
                        ?>
                        <input type="submit" name="sb" value="検索" class="btn btn-outline-secondary height">
                        <!-- <button class="btn btn-outline-secondary" type="button" id="">検索</button> -->
                    </form>
                </div>
            </div>
            ...
            // } else if ($count == 0) {
            //     echo "<p>該当する学生はいません。再度学生IDを入力して検索してください。</p>";
            // } else if (0 < $count) {
            ...
            <?php
            if ($count == 0) {
                echo "<h2>該当する学生はいません。再度学生IDを入力して検索してください。</h2>";
            }
            ?>
                ...
                <?php if ($count <= 0) { ?>
                    <?php foreach ($stmt as $row) { ?>
                        <tr>
                            ...
                            <td><a href="studentEd.php?page=<?php echo $row["student_id"]; ?>">編集</a></td>
                            <!--↑セキュリティを考えてあえて変数名をstudent_idではなくpageにしてみた-->
                        </tr>
                    <?php } ?>
                <?php } else if (0 < $count) { ?>
