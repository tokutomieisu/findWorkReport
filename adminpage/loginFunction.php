...
if (strcmp($uId, 'admin') == 0 && strcmp($uPass, 'admin') == 0) {     //等しいなら0
    header('Location: adminpage/index.html');   //管理者用ページを別フォルダに分けてると想定
    exit();     //以後の処理をしない
...
if (!empty($uId) && !empty($uPass)) {
    ...
    $sql = "SELECT * FROM mt_student WHERE no = :id";
    ...
    //合ってるかチェック
    if ($member) {
        if ($uPass == $member['pass']) {
            //ユーザ名をセッションに保存
            $_SESSION['USERID'] = $member['student_id'];
            $_SESSION['USERNAME'] = $member['name'];
            header('Location: ./search.php');
            exit();     //以後の処理をしない
        } 
    }else {
        $msg = "入力情報が違います。";      //具体的にどこが違うか書くとヒントになりそうなので…
    }
}
?>
